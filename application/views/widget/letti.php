	  ...
	  <div class="">
		 ...
		 <?php //var_dump ($newspiuletti); ?>
			...
			<?php foreach ($newspiuletti as $key=>$val) : ?>			
				 <li class="media"> <a href="<?php echo site_url(strtolower($val->categoria.'/'.$val->slug.'/'.$val->id)); ?>" class="media-photo" style="background-image:url(<?php echo ($val->allegati!="") ? base_url("images/news/".$val->allegati->url) : base_url('images/no_img.jpg'); ?>)"></a> <a href="<?php echo site_url(strtolower($val->categoria.'/'.$val->slug.'/'.$val->id)); ?>" class="media-date"><?php echo $val->data_ins['giorno']; ?><span><?php echo $val->data_ins['mese']; ?></span><?php echo $val->data_ins['anno']; ?></a>
